...
const showItemTooltip = item => {
    tooltip.show(item_type(item.type), message('tooltip-item-message', item.sprint_name, `<span class="date">${humanizeDate(new Date(item.date))}</span>`));
};

const showSprintTooltip = item => {
    tooltip.show(item.sprint_name, message('tooltip-sprint-message', `<span class="date">${humanizeDate(new Date(item.date))}</span>`, `<span class="date">${humanizeDate(new Date(item.end_date))}</span>`));
};
...
const zoomUpdate = (element, old_zoom) => {
    ...
    zoom.on("zoom.tooltip", tooltip.hide);
