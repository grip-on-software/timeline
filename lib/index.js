...
const formatData = (projects_data, filter) => {
    var result = [];
    Object.keys(projects_data).sort().forEach(project => {
...
const selectSprint = (element) => {
    ...
    if (lastSelectedProjectName !== d.project_name) {
        ...
        subtitle.text(locales.message('subchart-title', [d.project_name, '']));
    }
    ...
    fetchData(type => !type.subchart, d).then(project_data => {
        ...
        if (project_data) {
            ...
            fetchData((type) => type.subchart, d, start_subdata,
                ...
                if (currentData && lastSelectedProjectName === d.project_name) {
                ...
                burndown.create(subelement, project_data[0].name, project_data[0].data[0].sprint_id, d.end_date, locale);
            });
        }
        else {
            sprint_spinner.remove();
