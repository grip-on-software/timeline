...
loader.preload('data/sprint_start.json', require('../public/data/sprint_start.json'));
loader.preload('data/sprint_end.json', require('../public/data/sprint_end.json'));
...
const showItemTooltip = item => {
    ...
    tooltip.show(locales.attribute("types", item.type), message);
};
    ...
    fetchData(type => !type.subchart, d).then(project_data => {
        ...
        if (project_data) {
            ...
            fetchData((type) => type.subchart, d, start_subdata,
                ...
                burndown.create(subelement, project_data[0].name, project_data[0].data[0].sprint_id, earliest_start_date, last_end_date, d.end_date, locales);
            });
        }
        else {
            sprint_spinner.stop();
        ...
        .dropsDraw(function (drops, newDrops, scales, configuration, line_count, svg) {
            ...
            drops.each(function dropDraw(d, idx) {
                if (d.type == "sprint_start") {
                    ...
                    if (feature_label != '') {
                        rect.classed(!!features[d.project_name][d.sprint_id][feature_label] ? 'red' : 'green', true);
                        sprint.select('rect')
...
types.forEach(spec => {
    ...
    label.append('span').text(locales.attribute("types", t));
});
