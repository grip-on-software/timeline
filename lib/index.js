...
const types = [
    ...
    'red_metric', 'impediment', 'sprint_end', 'story'
];
...
const fetchData = (filter, callback) => {
    ...
    types.forEach((type) => {
        if (filter(type)) {
            var promise = loader.loadJSON("data/" + type + ".json", (partial_data) => {
...
const selectSprint = (element) => {
    ...
    sprint_spinner();
    fetchData((type) => true, (project_data) => {
...
const fillChart = () => {
    fetchData((type) => type_filter[type], (project_data) => {
