...
const sprintSpinner = new ui.Spinner({
...
const chartSpinner = new ui.Spinner({
...
const fetchData = (filter, info, targetData, collector) => {
    ...
    types.forEach((type) => {
        if (filter(type)) {
            ...
            var promise = loader.loadJSON(path, (partialData) => {
...
const dropsDraw = (drops, newDrops, scales, configuration, lineCount) => {
    ...
    var line = `sprint-lines-${lineCount}`;
    var sprints = d3.select(svg.parentNode).selectAll(`#${line}`).data([lineCount]);
    ...
    drops.each((d, idx) => {
        ...
        var id = `line-drop-${lineCount}-${idx}`;
